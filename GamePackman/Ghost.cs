    ...
    public class Ghost
    {
        ...
        public void Set_Ghosts()
        {
            ...
            for (int y = 0; y < 30; y++)
            {
                for (int x = 0; x < 27; x++)
                {
                    if (Game.gameBoard.Matrix[y, x] == 15)
                    {
                        Amount++;
                        xStart[Amount] = x;
                        yStart[Amount] = y;
        ...
        private bool direction_ok(int x, int y, int ghost)
        {
            ...
            if (Game.gameBoard.Matrix[y, x] < 4 || Game.gameBoard.Matrix[y, x] > 10) { return true; } else { return false; }
        }
        ...
        private void Other_Direction(int direction, int ghost)
        {
            ...
            if (Game.gameBoard.Matrix[yCoordinate[ghost], xCoordinate[ghost]] < 4)
            {
        ...
        public void CheckForPacman()
        {
            ...
            for (int x = 0; x < GhostAmount; x++)
            {
                if (xCoordinate[x] == Game.pacman.coordinateX && yCoordinate[x] == Game.pacman.coordinateY)
                {
                    switch (State[x])
                    {
                       // case 0: Game.player.LoseLife(); break;
                        case 1:
                            State[x] = 2;
                            hometimer.Enabled = true;
                            GhostImage[x].Image = Properties.Resources.eyes;
                         //   Game.player.UpdateScore(300);
                            break;
                    }
                }
            }
